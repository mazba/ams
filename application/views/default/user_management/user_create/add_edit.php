                    ...
                    <div class="portlet-body form">
                        <form id="system_save_form" action="<?php echo $CI->get_encoded_url('user_management/user_create/index/save'); ?>" method="post">
                            ...
                            <div class="form-body">
                                ...
                                <div class="form-group has-error row" >
                                    <div class="col-lg-2"><label class="control-label bold" for="name_bn"><?php echo $CI->lang->line('PROFILE_IMAGE'); ?></label></div>
                                    <div class="col-lg-8">
                                        <div id="imtext" style="width: 150px; height: 150px;float: left"><img src="<?php echo base_url()?>images/<?php echo $userInfo['picture_name']?'users/'.$userInfo['picture_name']:'profile.png';?>" style="width: 130px; height: 130px; " id="imtext" /></div>
                                        <input type="file" name="picture_name" id="picture_name" data-preview-container="#imtext" data-preview-height="30" class="validate[custom[validateMIME[image/jpeg|image/png|image/jpg|image/gif]]]" />
                                    </div>
                                </div>
                                <div class="form-group has-error row" >
                                    <div class="col-lg-2"><label class="control-label bold" for="name_bn"><?php echo $CI->lang->line('GENDER'); ?></label></div>
                                    <div class="col-lg-4">
                                        <select name="user_detail[gender]" class="form-control" >
                                            <?php
                                            $genders=$this->config->item('GENDER');
                                            foreach($genders as $gender_key=>$gender_text)
                                            {
                                                if($gender_key==$userInfo['gender'])
                                                {
                                                    ?>
                                                    <option value="<?php echo $gender_key;?>" selected="selected"><?php echo $gender_text;?></option>
                                                    <?php
                                                }
                                                else
                                                {
                                                    ?>
                                                    <option value="<?php echo $gender_key;?>"><?php echo $gender_text;?></option>
                                                    <?php
                                                }
                                            }
                                            ?>
